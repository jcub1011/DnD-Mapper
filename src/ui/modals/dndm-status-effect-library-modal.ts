...
export class DndmStatusEffectLibraryModal extends GameElement {
  ...
  override render(): TemplateResult {
    ...
    const filtered = query
      ? allTemplates.filter(
          (t) => t.name.toLowerCase().includes(query) || t.notes.toLowerCase().includes(query),
        )
      : allTemplates;
    ...
    return html`
      <dndm-modal
        .isOpen=${this.isOpen}
        .modalTitle=${"Status Effects & Conditions"}
          ...
          <div style="display: flex; flex-direction: column; gap: 8px;">
              ...
              ${filtered.map((t) => {
                ...
                return html`
                  ...
                  >
                    ...
                    ${
                      t.maxHpDelta != null
                        ? html`<div style="font-size: 0.7rem; color: var(--dndm-color-accent);">
                            Max HP: ${t.maxHpDelta >= 0 ? `+${t.maxHpDelta}` : t.maxHpDelta}
                          </div>`
                        : nothing
                    }
                    ${
                      t.onApplyHpDelta != null
                        ? html`<div style="font-size: 0.7rem; color: var(--dndm-color-crimson);">
                            On Apply HP:
                            ${t.onApplyHpDelta >= 0 ? `+${t.onApplyHpDelta}` : t.onApplyHpDelta}
                          </div>`
                        : nothing
                    }
                  </div>
            ...
            ${
              selected
                ? html`
                    <div
                      style="padding: 6px 8px; background: var(--dndm-color-surface-sunken); border-radius: var(--dndm-radius-sm); font-size: 0.8rem; border-left: 3px solid var(--dndm-color-accent);"
                    >
...
declare global {
  interface HTMLElementTagNameMap {
    "dndm-status-effect-library-modal": DndmStatusEffectLibraryModal;
  }
}
